...
export default class StructuralMetadataPlugin extends Plugin {
  ...
  async refreshCurrentFile(): Promise<void> {
    ...
    if (!file) {
      new Notice('Structural properties: no active file');
      return;
    }
  ...
  async refreshCurrentFolder(): Promise<void> {
    const file = this.getActiveFile();
    if (!file || !file.parent) {
      new Notice('Structural properties: no active file to determine a folder');
      return;
    }
    const paths = this.processor.getProcessableFilePathsInFolder(file.parent.path);
    if (paths.length === 0) {
      new Notice('Structural properties: no processable files in this folder');
      return;
    }
  ...
  async refreshEntireVault(): Promise<void> {
    ...
    if (paths.length === 0) {
      new Notice('Structural properties: no processable files found');
      return;
    }
    new Notice(`Structural properties: refreshing ${paths.length} file(s)…`);
    const result = await this.processPaths(paths);
  ...
  private reportResult(result: ProcessorResult, scope: string): void {
    if (result.errors > 0) {
      ...
      new Notice(
        `Structural properties (${scope}): ${result.applied} change(s) applied`,
      );
    }
  }
}
